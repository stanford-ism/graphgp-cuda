...
#define DISPATCH_K_DIM(DEST, FUNC) \
    if (n_dim == 1) { \
        ...
        else if (k <= 64) DEST = FUNC<64, 1>; \
        else throw std::runtime_error("only compiled for k <= 64"); \
    } else if (n_dim == 2) { \
        if      (k <= 4)  DEST = FUNC<4,  2>; \
        else if (k <= 8)  DEST = FUNC<8,  2>; \
        else if (k <= 16) DEST = FUNC<16, 2>; \
        else if (k <= 32) DEST = FUNC<32, 2>; \
        else if (k <= 64) DEST = FUNC<64, 2>; \
        else throw std::runtime_error("only compiled for k <= 64"); \
    } else if (n_dim == 3) { \
...
Error refine_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> points, // (N, d)
    Buffer<S64> neighbors, // (N - n0, k)
    Buffer<S64> offsets, // (B,) marking the ends of each batch
    Buffer<F64> cov_bins, // (R,)
    ...
    decltype(&refine<1,1,int64_t,double>) dispatch = nullptr;
    DISPATCH_K_DIM(dispatch, refine);
    dispatch(
        stream,
        points.typed_data(),
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    refine_ffi_64, refine_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>() // points
        .Arg<Buffer<S64>>() // neighbors
        .Arg<Buffer<S64>>() // offsets
        .Arg<Buffer<F64>>() // cov_bins
...
Error refine_transpose_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> points, // (N, d) in topological order
    Buffer<S64> neighbors, // (N - n0, k) in original order
    Buffer<S64> offsets, // (L,) marking the ends of each batch
    Buffer<F64> cov_bins, // (R,)
    Buffer<F64> cov_vals, // (B1, B2, ..., R)
    Buffer<F64> values, // (B1, B2, ..., N)
    ResultBuffer<F64> initial_values, // (B1, B2, ..., n0)
    ResultBuffer<F64> xi, // (B1, B2, ..., N - n0)
    ResultBuffer<F64> values_buffer // (B1, B2, ..., N) to use as a temporary buffer
) {
    size_t n_points = points.dimensions()[0];
    size_t n_dim = points.dimensions()[1];
    size_t n_levels = offsets.dimensions()[0];
    ...
    decltype(&refine_transpose<1,1,int64_t,double>) dispatch = nullptr;
    DISPATCH_K_DIM(dispatch, refine_transpose);
    dispatch(
        stream,
        points.typed_data(), 
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    refine_transpose_ffi_64, refine_transpose_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>() // points
        .Arg<Buffer<S64>>() // neighbors
        ...
        .Ret<Buffer<F64>>() // initial_values
        .Ret<Buffer<F64>>() // xi
        .Ret<Buffer<F64>>() // values_buffer
);

// ---------------------------------------------------------------------------------

Error refine_jvp_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> points, // (N, d)
    Buffer<S64> neighbors, // (N - n0, k)
    Buffer<S64> offsets, // (B,) marking the ends of each batch
    ...
    decltype(&refine_jvp<1,1,int64_t,double>) dispatch = nullptr;
    DISPATCH_K_DIM(dispatch, refine_jvp);
    dispatch(
        stream,
        points.typed_data(),
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    refine_jvp_ffi_64, refine_jvp_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>() // points
        .Arg<Buffer<S64>>() // neighbors
...
Error refine_vjp_ffi_impl_64(
    ...
    Buffer<F64> xi, // (B1, B2, ..., N - n0)
    Buffer<F64> values, // (B1, B2, ..., N)
    Buffer<F64> values_tangent, // (B1, B2, ..., N)
    ResultBuffer<F64> cov_vals_tangent, // (B1, B2, ..., R)
    ResultBuffer<F64> initial_values_tangent, // (B1, B2, ..., n0)
    ResultBuffer<F64> xi_tangent, // (B1, B2, ..., N - n0)
    ResultBuffer<F64> values_tangent_buffer // (B1, B2, ..., N) to use as a temporary buffer
) {
    size_t n_points = points.dimensions()[0];
    size_t n_dim = points.dimensions()[1];
    size_t n_levels = offsets.dimensions()[0];
    ...
    decltype(&refine_vjp<1,1,int64_t,double>) dispatch = nullptr;
    DISPATCH_K_DIM(dispatch, refine_vjp);
    dispatch(
        stream,
        points.typed_data(),
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    refine_vjp_ffi_64, refine_vjp_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>() // points
        .Arg<Buffer<S64>>() // neighbors
        ...
        .Arg<Buffer<F64>>() // xi
        .Arg<Buffer<F64>>() // values
        .Arg<Buffer<F64>>() // values_tangent
        .Ret<Buffer<F64>>() // cov_vals_tangent
        .Ret<Buffer<F64>>() // initial_values_tangent
        .Ret<Buffer<F64>>() // xi_tangent
        .Ret<Buffer<F64>>() // values_tangent_buffer
);

// ---------------------------------------------------------------------------------

Error refine_inv_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> points, // (N, d)
    Buffer<S64> neighbors, // (N - n0, k)
    Buffer<S64> offsets, // (B,) marking the ends of each batch
    ...
    decltype(&refine_inv<1,1,int64_t,double>) dispatch = nullptr;
    DISPATCH_K_DIM(dispatch, refine_inv);
    dispatch(
        stream,
        points.typed_data(),
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    refine_inv_ffi_64, refine_inv_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>() // points
        .Arg<Buffer<S64>>() // neighbors
...
Error refine_logdet_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> points, // (N, d)
    Buffer<S64> neighbors, // (N - n0, k)
    Buffer<S64> offsets, // (B,) marking the ends of each batch
    ...
    decltype(&refine_logdet<1,1,int64_t,double>) dispatch = nullptr;
    DISPATCH_K_DIM(dispatch, refine_logdet);
    dispatch(
        stream,
        points.typed_data(),
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    refine_logdet_ffi_64, refine_logdet_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>() // points
        .Arg<Buffer<S64>>() // neighbors
        ...
        .Arg<Buffer<F64>>() // cov_bins
        .Arg<Buffer<F64>>() // cov_vals
        .Ret<Buffer<F64>>() // logdet
);

// ==================== GRAPH CONSTRUCTION ====================

Error build_tree_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> points_in, // (N, d)
    ResultBuffer<F64> points, // (N, d)
    ResultBuffer<S64> split_dims, // (N,)
    ResultBuffer<S64> indices, // (N,)
    ResultBuffer<S64> tags, // (N,)
    ResultBuffer<F64> ranges // (N,)
) {
    size_t n_points = points_in.dimensions()[0];
    size_t n_dim = points_in.dimensions()[1];
    cudaMemcpyAsync(points->typed_data(), points_in.typed_data(), n_points * n_dim * sizeof(double), cudaMemcpyDeviceToDevice, stream);
    build_tree(
        stream,
        points_in.typed_data(),
        points->typed_data(),
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    build_tree_ffi_64, build_tree_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>() // points_in
        .Ret<Buffer<F64>>() // points
        .Ret<Buffer<S64>>() // split_dims
        .Ret<Buffer<S64>>() // indices
        .Ret<Buffer<S64>>() // tags
        .Ret<Buffer<F64>>() // ranges
);

// ---------------------------------------------------------------------------------

Error query_neighbors_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> points, // (N, d)
    Buffer<S64> split_dims, // (N,) split dimensions
    Buffer<S64> query_indices, // (Q,)
    Buffer<S64> max_indices, // (Q,)
    ResultBuffer<S64> neighbors // (Q, k)
) {
    size_t n_points = points.dimensions()[0];
    size_t n_queries = query_indices.dimensions()[0];
    size_t n_dim = points.dimensions()[1];
    ...
    decltype(&query_neighbors_kernel<1,1,int64_t,double>) dispatch = nullptr;
    DISPATCH_K_DIM(dispatch, query_neighbors_kernel);
    dispatch<<<cld(n_queries, 256), 256, 0, stream>>>(
        points.typed_data(), 
        split_dims.typed_data(),
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    query_neighbors_ffi_64, query_neighbors_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>() // points
        .Arg<Buffer<S64>>() // split_dims
        .Arg<Buffer<S64>>() // query_indices
        .Arg<Buffer<S64>>() // max_indices
        .Ret<Buffer<S64>>() // neighbors
);

// ---------------------------------------------------------------------------------

Error query_preceding_neighbors_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> points, // (N, d)
    Buffer<S64> split_dims, // (N,) split dimensions
    ResultBuffer<S64> neighbors // (N - n0, k)
) {
    size_t n_points = points.dimensions()[0];
    size_t n_dim = points.dimensions()[1];
    size_t n0 = n_points - neighbors->dimensions()[0];
    ...
    decltype(&query_preceding_neighbors_kernel<1,1,int64_t,double>) dispatch = nullptr;
    DISPATCH_K_DIM(dispatch, query_preceding_neighbors_kernel);
    dispatch<<<cld(n_points - n0, 256), 256, 0, stream>>>(
        points.typed_data(), 
        split_dims.typed_data(),
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    query_preceding_neighbors_ffi_64, query_preceding_neighbors_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>() // points
        .Arg<Buffer<S64>>() // split_dims
...
Error compute_depths_parallel_ffi_impl_64(
    cudaStream_t stream,
    Buffer<S64> neighbors, // (N - n0, k)
    ResultBuffer<S64> depths, // (N,)
    ResultBuffer<S64> temp // (N,)
) {
    size_t n_points = depths->dimensions()[0];
    size_t n0 = n_points - neighbors.dimensions()[0];
    size_t k = neighbors.dimensions()[1];
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    compute_depths_parallel_ffi_64, compute_depths_parallel_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<S64>>() // neighbors
        .Ret<Buffer<S64>>() // depths
        .Ret<Buffer<S64>>() // temp
);

// ---------------------------------------------------------------------------------

Error order_by_depth_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> points_in, // (N, d)
    Buffer<S64> indices_in, // (N,)
    Buffer<S64> neighbors_in, // (N - n0, k)
    Buffer<S64> depths_in, // (N,)
    ResultBuffer<F64> points_out,
    ResultBuffer<S64> indices_out,
    ResultBuffer<S64> neighbors_out,
    ResultBuffer<S64> depths_out,
    ResultBuffer<S64> temp // (2*N,)
) {
    size_t n_points = points_in.dimensions()[0];
    size_t n_dim = points_in.dimensions()[1];
    size_t n0 = n_points - neighbors_in.dimensions()[0];
    ...
    cudaMemcpyAsync(points_out->typed_data(), points_in.typed_data(), n_points * n_dim * sizeof(double), cudaMemcpyDeviceToDevice, stream);
    cudaMemcpyAsync(indices_out->typed_data(), indices_in.typed_data(), n_points * sizeof(int64_t), cudaMemcpyDeviceToDevice, stream);
    cudaMemcpyAsync(neighbors_out->typed_data(), neighbors_in.typed_data(), (n_points - n0) * k * sizeof(int64_t), cudaMemcpyDeviceToDevice, stream);
    cudaMemcpyAsync(depths_out->typed_data(), depths_in.typed_data(), n_points * sizeof(int64_t), cudaMemcpyDeviceToDevice, stream);
    ...
    order_by_depth(
        ...
        temp->typed_data(),
        temp->typed_data() + n_points,
        reinterpret_cast<double*>(temp->typed_data() + n_points),
        n0,
        k,
        n_points,
        n_dim
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    order_by_depth_ffi_64, order_by_depth_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>() // points_in
        .Arg<Buffer<S64>>() // indices_in
        .Arg<Buffer<S64>>() // neighbors_in
        .Arg<Buffer<S64>>() // depths_in
        .Ret<Buffer<F64>>() // points_out
        .Ret<Buffer<S64>>() // indices_out
        .Ret<Buffer<S64>>() // neighbors_out
        .Ret<Buffer<S64>>() // depths_out
        .Ret<Buffer<S64>>() // temp
);

// ---------------------------------------------------------------------------------

Error build_graph_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> points_in,
    ResultBuffer<F64> points_out, // (N, d)
    ResultBuffer<S64> indices_out, // (N,)
    ResultBuffer<S64> neighbors_out, // (N - n0, k)
    ResultBuffer<S64> depths_out, // (N,)
    ResultBuffer<S64> temp_out // (2*N,)
) {
    size_t n_points = points_in.dimensions()[0];
    size_t n_dim = points_in.dimensions()[1];
    size_t n0 = n_points - neighbors_out->dimensions()[0];
    ...
    double* points = points_out->typed_data();
    int64_t* indices = indices_out->typed_data();
    int64_t* neighbors = neighbors_out->typed_data();
    int64_t* depths = depths_out->typed_data();
    int64_t* temp_int = temp_out->typed_data();
    double* temp_float = reinterpret_cast<double*>(temp_out->typed_data());

    // Build tree
    cudaMemcpyAsync(points, points_in.typed_data(), n_points * n_dim * sizeof(double), cudaMemcpyDeviceToDevice, stream);
    build_tree(
        stream,
        points_in.typed_data(),
        points,
    ...
    // Query preceding neighbors
    decltype(&query_preceding_neighbors_kernel<1,1,int64_t,double>) query_dispatch = nullptr;
    DISPATCH_K_DIM(query_dispatch, query_preceding_neighbors_kernel);
    query_dispatch<<<cld(n_points - n0, 256), 256, 0, stream>>>(points, depths, neighbors, n0, k, n_points - n0);
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    build_graph_ffi_64, build_graph_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>()  // points_in
        .Ret<Buffer<F64>>()  // points
...
Error sort_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> keys_in,
    ResultBuffer<F64> keys_out
) {
    size_t n = keys_in.dimensions()[0];
    cudaMemcpyAsync(keys_out->typed_data(), keys_in.typed_data(), n * sizeof(double), cudaMemcpyDeviceToDevice, stream);
    sort(keys_out->typed_data(), n, stream);
    return Error::Success();
}

XLA_FFI_DEFINE_HANDLER_SYMBOL(
    sort_ffi_64, sort_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>()  // keys_in
        .Ret<Buffer<F64>>()  // keys_out
);

Error sort_three_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> keys1_in,
    Buffer<F64> keys2_in,
    Buffer<F64> keys3_in,
    ResultBuffer<F64> keys1_out,
    ResultBuffer<F64> keys2_out,
    ResultBuffer<F64> keys3_out
) {
    size_t n = keys1_in.dimensions()[0];
    cudaMemcpyAsync(keys1_out->typed_data(), keys1_in.typed_data(), n * sizeof(double), cudaMemcpyDeviceToDevice, stream);
    cudaMemcpyAsync(keys2_out->typed_data(), keys2_in.typed_data(), n * sizeof(double), cudaMemcpyDeviceToDevice, stream);
    cudaMemcpyAsync(keys3_out->typed_data(), keys3_in.typed_data(), n * sizeof(double), cudaMemcpyDeviceToDevice, stream);
    sort(keys1_out->typed_data(), keys2_out->typed_data(), keys3_out->typed_data(), n, stream);
    return Error::Success();
}

XLA_FFI_DEFINE_HANDLER_SYMBOL(
    sort_three_ffi_64, sort_three_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>()  // keys1_in
        .Arg<Buffer<F64>>()  // keys2_in
        .Arg<Buffer<F64>>()  // keys3_in
        .Ret<Buffer<F64>>()  // keys1_out
        .Ret<Buffer<F64>>()  // keys2_out
        .Ret<Buffer<F64>>()  // keys3_out
);

Error sort_four_ffi_impl_64(
    cudaStream_t stream,
    Buffer<F64> keys1_in,
    Buffer<F64> keys2_in,
    Buffer<F64> keys3_in,
    ...
    ResultBuffer<F64> keys2_out,
    ResultBuffer<F64> keys3_out,
    ResultBuffer<F64> keys4_out
) {
    size_t n = keys1_in.dimensions()[0];
    cudaMemcpyAsync(keys1_out->typed_data(), keys1_in.typed_data(), n * sizeof(double), cudaMemcpyDeviceToDevice, stream);
    cudaMemcpyAsync(keys2_out->typed_data(), keys2_in.typed_data(), n * sizeof(double), cudaMemcpyDeviceToDevice, stream);
    cudaMemcpyAsync(keys3_out->typed_data(), keys3_in.typed_data(), n * sizeof(double), cudaMemcpyDeviceToDevice, stream);
    cudaMemcpyAsync(keys4_out->typed_data(), keys4_in.typed_data(), n * sizeof(double), cudaMemcpyDeviceToDevice, stream);
    sort(keys1_out->typed_data(), keys2_out->typed_data(), keys3_out->typed_data(), keys4_out->typed_data(), n, stream);
    return Error::Success();
}

XLA_FFI_DEFINE_HANDLER_SYMBOL(
    sort_four_ffi_64, sort_four_ffi_impl_64,
    Ffi::Bind()
        .Ctx<PlatformStream<cudaStream_t>>()
        .Arg<Buffer<F64>>()  // keys1_in
        .Arg<Buffer<F64>>()  // keys2_in
        .Arg<Buffer<F64>>()  // keys3_in
        .Arg<Buffer<F64>>()  // keys4_in
        .Ret<Buffer<F64>>()  // keys1_out
        .Ret<Buffer<F64>>()  // keys2_out
        .Ret<Buffer<F64>>()  // keys3_out
        .Ret<Buffer<F64>>()  // keys4_out
);
