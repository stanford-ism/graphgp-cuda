...
__forceinline__ __device__ void query_preceding_neighbors_alt_impl(
    ...
    uint32_t current = 0;
    uint32_t root_parent = (1 << uint32_t(-1)) - uint32_t(1);
    uint32_t previous = root_parent;
    uint32_t next = 0;

    // traverse until we return to root
    while (current != root_parent) {
        uint32_t level = floored_log2(current + 1);
        uint32_t parent = (i - ((1 << level) - uint32_t(1))) % (1 << (level - uint32_t(1))) + ((1 << (level - uint32_t(1))) - uint32_t(1));
        ...
        uint32_t near_child = current + (1 << (level + near_side));
        uint32_t far_child = current + (1 << (level + 1 - near_side));
...
__global__ void query_preceding_neighbors_alt_kernel(
    ...
    query_preceding_neighbors_alt_impl<MAX_K, N_DIM>(points, split_dims, neighbors, tid, k);
}
