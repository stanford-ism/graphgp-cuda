...
refine_vjp_linear_p = Primitive("hugegp_cuda_refine_vjp_linear")
...
def _initialize():
    ...
    refine_vjp_linear_p.def_impl(refine_vjp_linear_impl)
    refine_vjp_linear_p.def_abstract_eval(refine_vjp_linear_abstract_eval)
    refine_vjp_linear_p.multiple_results = True
    mlir.register_lowering(refine_vjp_linear_p, refine_vjp_linear_lowering, platform="gpu")  # type: ignore
    # batching.primitive_batchers[refine_vjp_linear_p] = refine_vjp_linear_batch
...
def refine_transpose(tangents_out, *primals):
    ...
    div_buffer, dx = refine_vjp_linear(p, o, n, cb, cv, dv)
...
@trace("refine_vjp_linear")
def refine_vjp_linear(points, offsets, neighbors, cov_bins, cov_vals, values_tangent):
    return refine_vjp_linear_p.bind(points, offsets, neighbors, cov_bins, cov_vals, values_tangent)


@trace("refine_vjp_linear_abstract_eval")
def refine_vjp_linear_abstract_eval(*args):
    p, o, n, cb, cv, dv = args
    div_buffer = jax.ShapeDtypeStruct(dv.shape, jnp.float32)
    dx = jax.ShapeDtypeStruct(dv.shape, jnp.float32)
    return div_buffer, dx


@trace("refine_vjp_linear_lowering")
def refine_vjp_linear_lowering(ctx, primals_in, tangents_out):
    return jax.ffi.ffi_lowering("hugegp_cuda_refine_vjp_linear_bind")(ctx, primals_in, tangents_out)


@trace("refine_vjp_linear_impl")
def refine_vjp_linear_impl(p, o, n, cb, cv, dv):
