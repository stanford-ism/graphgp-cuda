...
ffi::Error refine_xla_impl(
    ...
    if ((k < 2) || (k > 5)) return ffi::Error::InvalidArgument("only compiled for 2 <= k <= 5");
    if (n_dim > 3) return ffi::Error::InvalidArgument("only compiled for n_dim <= 3");
    
    if (n_dim != 2) return ffi::Error::InvalidArgument("only compiled for n_dim == 2");
    if (k != 4) return ffi::Error::InvalidArgument("only compiled for k == 4");

    refine<4, 2>(stream, points.typed_data(), offsets.typed_data(), neighbors.typed_data(), cov_bins.typed_data(), cov_vals.typed_data(), initial_values.typed_data(), xi.typed_data(), values->typed_data(), n_points, n_levels, n_cov);
...
ffi::Error refine_transpose_xla_impl(
    ...
    refine_transpose<4, 2>(stream, points.typed_data(), offsets.typed_data(), neighbors.typed_data(), cov_bins.typed_data(), cov_vals.typed_data(), values_tangent.typed_data(), initial_values_tangent->typed_data(), xi_tangent->typed_data(), n_points, n_levels, n_cov);
...
ffi::Error query_coarse_neighbors_xla_impl(
    ...
    if (k > 8) return ffi::Error::InvalidArgument("only compiled for k <= 8");

    if (n_dim == 1) {
        query_coarse_neighbors<8, 1>(stream, points.typed_data(), split_dims.typed_data(), neighbors->typed_data(), n_points, k);
    } else if (n_dim == 2) {
        query_coarse_neighbors<8, 2>(stream, points.typed_data(), split_dims.typed_data(), neighbors->typed_data(), n_points, k);
    } else if (n_dim == 3) {
        query_coarse_neighbors<8, 3>(stream, points.typed_data(), split_dims.typed_data(), neighbors->typed_data(), n_points, k);
    }
